  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.props.hotnovel !== prevProps.hotnovel) {
      ...
      for (let i = 0; i < 10; i++) {
        arr.push({
            key: i,
            novelsname: this.props.hotnovel.data[i].tentruyen,
            idnovel: this.props.hotnovel.data[i].idtruyen,
            lasterchapter: this.props.hotnovel.data[i].chuongmoi,
        });
      }
  ...
  render() {
    return (
      <div className="doc_nhieu">
        <h2 className="feature_title">Truyện Hay</h2>
        <ul>
        {this.state.docnhieu.map((docnhieu,key) => (
          <li>
            <span>{key}</span>{" "}
            <a
              href={'/truyen/'+docnhieu.idnovel+'/page/1'}
